...
public class Driver {
  ...
  public static void GeneratedDataPopulate() {
    ...
    Date[] randDates = genRandDates(amount);
    ...
    //Now try to do a Binary Search for a specific Observation
    int target = 24;
    LocalDate localDate = convertToLocalDateViaInstant(randDates[target]);
    Observation searchObs = new Observation(localDate, reporterList.get(target));
    System.out.println("\nSearching for a observation with local date of: " + localDate);
    int index3 = Collections.binarySearch(obsList, searchObs);
    System.out.println("index3:"+ index3);
    if(index >= 0)
      System.out.println(obsList.get(index3));
    else
      System.out.println("The observation with local date of : "+localDate+" was not found");
  ...
  private static List<SightingState> genSightingStates(int amount, double[] windSpeeds, double[] sightingDistances) {
    ...
    for (int i = 0; i < amount; i++) {
      SightingState ss = new SightingState();
      ss.setWindSpeed((float) windSpeeds[i]);
      ss.setSightingDistance((int) sightingDistances[i]);
      ss.setSightingPlatform(faker.address().city());
      ssList.add(ss);
    }
    return ssList;
  }
   ...
   */
  private static Date[] genRandDates(int amount) {
    Date[] randDates = new Date[amount];
    for (int i = 0; i < amount; i++) {
      Random rnd;
      Date dt;
      long ms;
